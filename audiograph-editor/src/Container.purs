...
component =
  ...
  renderPlayer ::  ∀ eff1. State -> H.ParentHTML Query ChildQuery ChildSlot (Aff (wau :: WebAudio | eff1))
  renderPlayer state =
    case state.graphResult of
      Right audioGraph ->
        HH.div
          ...
          [ HH.slot' playerSlotNo unit (Button.component "play") unit (HE.input HandlePlayButton)]
      Left err ->
        HH.div_
          [  ]
  ...
  eval (HandleSampleButton (Button.Toggled _) next) = do
    _ <- H.query' editorSlotNo unit $ H.action (ED.UpdateContent cowbell)
    pure next
